...
void bfs_omp_mpi_2d(Graph graph, solution* sol)
{
  ...
  bool update = false;
  // {{{2 初始化xk和visited(就是src点), 相当于第一次迭代
...
void bfs_omp_mpi_bottom_up(Graph graph, solution* sol)
{
  ...
  MPI_Comm_size(MPI_COMM_WORLD, &nprocs);  
    ...
    // {{{2 计算负责的范围[my_start,my_end), 其中最后一个进程少负责一些
  ...
  recvcounts[i]=n_min;
    ...
    #pragma omp parallel for reduction(|:update)
        for (int i=my_start; i < my_end; i++) {                   
            if (frontier[i] == -1) {
                ...
                for(int neighbor = start_edge; neighbor < end_edge; neighbor++) {
                    ...
                    if(frontier[incoming] == iteration-1) { // 事实上, 应该绝不可能>=0, 而不等于iteration-1
        ...
        if(!update) break;
        MPI_Allgatherv(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL,frontier, recvcounts, displs, MPI_INT, MPI_COMM_WORLD);
