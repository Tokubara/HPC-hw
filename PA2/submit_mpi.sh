...
for((i=0;i<${#config[@]};i+=2)) do
  ...
  export OMP_NUM_THREADS=$T
	...
	srun -N $N -n $n ./bfs_omp_mpi $ass/graph/${1}m.graph >> $result_file # 因为main中去掉了printf的\n
	# mpirun --oversubscribe -np $P ./bfs_omp_mpi $ass/graph/500k.graph >> $result_file # 因为main中去掉了printf的\n
done
echo done
